...
export abstract class AbstractName implements Name{
    ...
    public concat(other: Name): Name {
        ...
        for(let i = 0; i < other.getNoComponents(); i++) {
            newObject.append(other.getComponent(i).replaceAll(this.regex, ESCAPE_CHARACTER + this.delimiter));
        }
        if(this.getNoComponents() !== length) {
            throw new MethodFailedException("Concatenation failed");
        }
        (newObject as AbstractName).assertCorrectEscapedName();
        return newObject;
    }
    ...
    protected assertIsNotNullOrUndefined(value: any): void {
        IllegalArgumentException.assert(value !== null && value !== undefined);
    }
}